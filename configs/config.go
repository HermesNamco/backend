...
func LoadConfig() (*Config, error) {
	...
	config := &Config{
		...
		Stock: StockAPIConfig{
			ProviderType: getEnv("STOCK_PROVIDER_TYPE", string(stock.DefaultProvider)),
	...
	if config.Stock.ProviderType != "" {
		...
		switch stock.ProviderType(config.Stock.ProviderType) {
		case stock.TuShareProvider, stock.TonghuashunProvider:
		...
		if isKnownProvider {
			...
			if stock.ProviderType(config.Stock.ProviderType) == stock.TonghuashunProvider && config.Stock.APISecret == "" {
