...
func (s *MomentumStrategy) Backtest(ctx context.Context, stockData []models.Stock, initialCapital float64) (*BacktestResult, error) {
	...
	if atrValues == nil && s.atrStopMultiplier > 0 { 
		...
		for i := range atrValues { atrValues[i] = sortedData[i].Close * 0.02 } // Fallback to 2% of close price
	}
		...
		if currentPosition == nil { // Check if position was closed by SL
		    if currentSignals, ok := signalsByTime[bar.Timestamp]; ok {
			    for _, sig := range currentSignals {
				    if sig.StockCode != bar.Code { continue }
	...
	for _, tr := range trades { if tr.ProfitLoss > 0 { winCount++ } else if tr.ProfitLoss < 0 { lossCount++ } }
	...
	if len(returns) > 1 {
		...
		for _, r := range returns { sumReturns += r }
		meanReturn := sumReturns / float64(len(returns))
		for _, r := range returns { sumSqDiff += math.Pow(r-meanReturn, 2) }
